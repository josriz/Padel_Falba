...
export default function TournamentBracket({ registrations = [] }) {
  const [teams, setTeams] = useState(registrations.map(r => `${r.first_name} ${r.last_name}`));
  const [round16, setRound16] = useState(() => generateInitialRound(teams.length));
  const [round8, setRound8] = useState([]);
  const [round4, setRound4] = useState([]);
  const [round2, setRound2] = useState([]);

  useEffect(() => {
    setTeams(registrations.map(r => `${r.first_name} ${r.last_name}`));
    setRound16(generateInitialRound(registrations.length));
    setRound8(Array(Math.ceil(registrations.length / 4)).fill(null).map(() => ({ ...initialMatch })));
    setRound4(Array(Math.ceil(registrations.length / 8)).fill(null).map(() => ({ ...initialMatch })));
    setRound2(Array(Math.ceil(registrations.length / 16)).fill(null).map(() => ({ ...initialMatch })));
  }, [registrations]);
  ...
  function Match({ match, teamNameA, teamNameB, onScoreChange, matchIndex, round, setRound }) {
    return (
      <div style={{ display: "flex", gap: 12, marginBottom: 12, alignItems: "center" }}>
        ...
        {match.winner !== null && (
          <span style={{ marginLeft: "auto" }}>Vincitore: {teams[match.winner] || "bye"}</span>
        )}
      </div>
    );
  }
    ...
    <div style={{ maxWidth: 720, margin: "20px auto", fontFamily: "Arial, sans-serif" }}>
      ...
      {round16.map((match, i) => (
        <Match
          key={`r16-${i}`}
          match={match}
          teamNameA={teams[match.aIndex]}
          teamNameB={teams[match.bIndex]}
